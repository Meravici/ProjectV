...
public class ConstructorMoc implements ConstructorAPI {
    ...
    public ConstructorMoc(ProjectV activity){
        ...
        groups.get(0).addTask(tasks[0]);
        groups.get(0).addTask(tasks[1]);

        groups.get(1).addTask(tasks[0]);
        groups.get(1).addTask(tasks[1]);
    }
    ...
    public void registerGroup(String name) {
        Group group = new Group(123,name,null,new Random().nextInt(3));
        storageListener.onGroupCreated(group);
    }
